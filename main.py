...
def detect_people(frame):
    ...
    Returns list of (x,y,w,h) for each detected person.
    """
...
class Track:
    def __init__(self, bbox, feature, frame_idx, video_id):
    ...
    while True:
        ...
        for (x, y, w, h) in detections:
            ...
            for t in tracks:
                ...
                # IoU
                tx, ty, tw, th = t.bbox
                ix1 = max(tx, x); iy1 = max(ty, y)
                ix2 = min(tx+tw, x+w); iy2 = min(ty+th, y+h)
                ...
                if iou > 0.3:
                    ...
                    if sim > 0.3 and iou > best_iou:
                        best_iou, best = iou, t
            if best:
                best.update((x,y,w,h), feat, frame_idx)
                track = best
            else:
                track = Track((x,y,w,h), feat, frame_idx, video_id)
                tracks.append(track)
